...
class Campaign(models.Model):
    ...
    convCount = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    convSales = models.IntegerField(blank=True, null=True)

    def __str__(self) -> str:
        return f"{self.campaignNo} - {self.campaign_name}"
...
class AdSet(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.adSetNo} - {self.adset_name}"
...
class Creative(models.Model):
    ...
    ad_set = models.ForeignKey(
        AdSet, on_delete=models.CASCADE, related_name="creatives"
    )
    account = models.ForeignKey(Account, on_delete=models.CASCADE)
    ...
    def __str__(self) -> str:
        return f"{self.creativeNo} - {self.creative_name}"
